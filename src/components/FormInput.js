...
const FormInput = (props) => {
  ...
  const showPassword = (e) => {
    e.target.offsetParent.children[1].type === "password"
      ? (e.target.offsetParent.children[1].type = "text")
      : (e.target.offsetParent.children[1].type = "password")
  }
  ...
  return (
    <div className="form-input name-input mb-4">
      ...
      {props.name === "password_repeat" && (
        <i className="fas fa-eye" onClick={showPassword}></i>
      )}
      <small className="text-danger" id={`${name}` + "-error"}>
        {errorMessage}
      </small>
    </div>
  )
